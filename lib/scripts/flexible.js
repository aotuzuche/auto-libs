...
(function(window, lib) {
  ...
  window.isAlipay = /AlipayClient/gi.test(ua);
  window.isSmallSwan = /swan/gi.test(ua);

  if (window.isWX || window.isAlipay || window.isSmallSwan) {
    ...
    var atMiniProgram = (/[?&]atMiniProgram=(.*?)(&|$)/.exec(search) || [])[1];
    if (atMiniProgram) {
      ss.setItem('_app_atMiniProgram_', atMiniProgram);
    }
  }
